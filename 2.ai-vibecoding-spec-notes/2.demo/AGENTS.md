...
默认采用 pnpm workspace + Turborepo Monorepo：`packages/*` 放共享包，`apps/frontend/*` 放前端应用，`apps/backend/*` 放后端应用。共享类型和 UI 能力进入独立包；前端通过 `pages`、`components`、`hooks`、`services`、`types`、`router` 分层；后端按领域模块组织 `module/controller/service/dto`，实体集中管理。

后端 API 默认使用统一路由前缀、明确的认证方案、DTO 校验、统一异常处理和 API 文档。不同服务实现同一契约时，必须保持接口、鉴权和数据模型一致。
